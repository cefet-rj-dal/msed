...
multi_scale_event_detect <- function(serie=NULL, noise.amp = 6.4e-07 , trials = 100, limit_ri_sup = 2.14, limit_ri_inf = 1.50, frequency_date=12, start_date=c(2004,1), stop_rule="type1", name_of_columns=list(), type_events = list("anomaly", "cp", "cp_variance"), rf_dataframe=NULL , soft_window = 15){
    ...
    metrics <- soft_evaluate(events = events, reference = rf_dataframe)
    return(metrics)
}
