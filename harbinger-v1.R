...
evtdet.seminalChangePoint <- function(data,...){
  ...
  changepoints_v1 <- function(data,w=100,na.action=na.omit,...){
    ...
    anomalies <- data.frame(time=data[index.cp,"time"],serie=serie_name, type="change point")
    names(anomalies) <- c("time","serie","type")
...
evaluate <- function(events, reference, 
  ...
  metric_value <- c("accuracy" = hardMetrics$accuracy,
...
soft_evaluate <- function(events, reference, k=15,
  ...
  metric_value <- c("accuracy" = softMetrics$accuracy,
