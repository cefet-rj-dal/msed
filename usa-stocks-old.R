...
stocks <- stocks[stocks_usa_name_columns]
# # cpm change points
...
df_domain <- data.frame(name_domain_group, SFVE, SSCP, SSVE, LT, AD)
...
for (name_column in stocks_usa_name_columns) {
  ...
  anomalies_trend <- get_anomalyze_trend(stocks_tmp[["Data"]], overall_adaptative_trend, long_term_price)
  ...
  df_short_term_fluctuation <- data.frame(time=stocks$Data, serie=short_term_fluctuation)
  ...
  SSCP <- evtdet.seminalChangePoint2(df_shock_from_significant_events, w=120,mdl=linreg,na.action=na.omit)
  ...
  break_s_whithout_windowm <- append(break_s_whithout_windowm, as.Date( if(LT != 0) LT$time else NA, format="%Y-%m-%d"))
...
file_metrics <- sprintf("files/stocks_usa-msed-025-%s.csv", "metrics")
file_domain_groups <- sprintf("files/stocks_usa-msed-025-%s.csv", "domain_groups")
