...
install.packages("locfit", repos = "http://cran.r-project.org")
install.packages("ICSS", repos = "http://cran.r-project.org")
install.packages("EMD", repos = "http://cran.r-project.org")

install.packages("hht", repos = "http://cran.r-project.org")
...
currencies$Data <- as.Date(as.yearmon(currencies$Data))
# currencies <- currencies[currencies$Data < "2019-01-01",]
...
currencies <- subset(currencies, !is.na(strtoi(trim(as.numeric(as.POSIXct(currencies$Data, format = "%Y-%m"))))))
time_series <- c(time_series, strtoi(trim(as.numeric(as.POSIXct(currencies$Data, format = "%Y-%m")))))
noise.amp <- 6.4e-9
trials <- 100
...
for (name_column in currencies_name_columns) {
  ...
  currencies_tmp <- currencies
  results <- NULL
  ea <- NULL
  results <- hht::CEEMD(currencies_tmp[[name_column]], time_series, noise.amp, trials, max.sift = 200, boundary = "wave", stop.rule = "type5")
  ...
  ############## Variance change #########################
  breakpoints_short_term_fluctuation <- get_break_dates(ICSS(short_term_fluctuation), currencies_tmp)
  breakpoints_volatility_events <- get_break_dates(ICSS(volatility_events), currencies_tmp)
  breakpoints_shock_from_siginificant_events <- get_break_dates(ICSS(shock_from_significant_events), currencies_tmp)
  
  ############## TREND ###############################
  
  anomalies <-get_anomalyze(currencies_tmp[["Data"]], short_term_fluctuation, shock_from_significant_events, volatility_events)
  anomalies_trend <- get_anomalyze_trend(currencies_tmp[["Data"]], df_type, overall_adaptative_trend, long_term_price)
  
  
  print(name_column)
  df <- data.frame(time = currencies_tmp$Data, x = short_term_fluctuation)
  
  reference <- currencies_tmp[c("Data", experimento)]
  
  
  df_short_term_fluctuation_events <- data.frame(time=currencies_tmp$Data, serie=short_term_fluctuation)
  STCP <- evtdet.seminalChangePoint2(df_short_term_fluctuation_events, w=80,mdl=linreg,na.action=na.omit)
  
  
  df_shock_from_significant_events <- data.frame(time=currencies_tmp$Data, serie=shock_from_significant_events)
  SSCP <- evtdet.seminalChangePoint2(df_shock_from_significant_events, w=80,mdl=linreg,na.action=na.omit)
  
  df_long_term_price_events<- data.frame(time=currencies_tmp$Data, serie=long_term_price)
  tryCatch(
    expr = {
      LT <- evtdet.seminalChangePoint2(df_long_term_price_events, w=80,mdl=linreg,na.action=na.omit)
    },
    error = function(e){ 
  ...
  if(length(breakpoints_short_term_fluctuation) > 0){
    SFVE_soft <-  soft_evaluate_components(get_events(breakpoints_short_term_fluctuation),currencies_tmp)
  }
  if(nrow(STCP) > 0){
    STCP_soft <-  soft_evaluate_components(get_events(STCP$time),currencies_tmp)
  }
  
  if(nrow(SSCP) > 0){
    SSCP_soft <-  soft_evaluate_components(get_events(SSCP$time),currencies_tmp)
  }
  
  if(length(breakpoints_shock_from_siginificant_events) > 0){
    SSVE_soft <-  soft_evaluate_components(get_events(breakpoints_shock_from_siginificant_events), currencies_tmp)
  }
  
  if(nrow(LT) > 0){
    LT_soft <-  soft_evaluate_components(get_events(LT$time), currencies_tmp)
  }
  
  if(length(anomalies) > 0){
    AD_soft <-  soft_evaluate_components(get_events(anomalies), currencies_tmp)
  }
  ...
  df <- NULL
  events <- data.frame(time, serie, type)
  df <- data.frame(time = currencies_tmp$Data, x = currencies_tmp[[name_column]])
  metric <- c(
...
file_metrics <- sprintf("files/currencies-msed-exp1-%s.csv", "metrics")
file_domain_groups <- sprintf("files/currencies-msed-exp1-%s.csv", "domain_groups")
